    ...
    class Inbox:
        ...
        def emails(self):
            ...
            return [
                ...
                for path in sorted((Path(self.source) / "inbox").glob("email_*.json"))
            ]
        ...
        def get(self, email_id):
            if self.is_http:
                with urllib.request.urlopen(
                    self.source + "/emails/" + email_id
                ) as response:
                    return json.load(response)
            return json.loads(
                (Path(self.source) / "inbox" / (email_id + ".json")).read_text(
                    encoding="utf-8"
                )
            )
