...
def draft_correction_email(case, *, confirmed=False):
    ...
    lines = [
        "Dear Team,",
        "",
        "During verification of the draft Bill of Lading against the Shipping Instruction, we found:",
    ]
    for mismatch in case["mismatches"]:
        lines.append(
            f"- {mismatch['field']}: SI says {mismatch['si']['raw_value']}; draft BL says {mismatch['bl']['raw_value']}."
        )
    lines.extend(
        ["", "Please review and amend the draft accordingly.", "", "Best regards,"]
    )
    return "\n".join(lines)
...
class RaiseIssueToHuman:
    ...
    def showIssue(self, case=None):
        ...
        return {
            ...
            "reason": case.get("internal_reason") or case.get("review_reason"),
            "evidence": case.get("mismatches")
            or case.get("missing_fields")
            or case.get("error"),
        }
