...
def submission_for_case(case):
    ...
    return {"category": case["category"], "status": status,
            "review_reason": (case.get("review_reason") or "unreadable") if status == "NEEDS_REVIEW" else None,
...
def confirm_value(case, role, field):
    ...
    if not value or value.get("normalized_value") is None:
        raise ValueError("There is no extracted value to confirm; enter a correction instead")
    value["confidence"] = 1
...
def _recompare(case):
    ...
    if checks and (not checks.get("pairing", {}).get("valid", True) or
        ...
        reason = ("POSSIBLE_WRONG_DOCUMENT_PAIR" if not checks.get("pairing", {}).get("valid", True)
                  else "DOCUMENT_INTERNAL_INCONSISTENCY")
        case.update(status="NEEDS_REVIEW", review_reason="wrong_doc_type", internal_reason=reason)
        return
    if comparison["missing_fields"]:
        ...
        return
    uncertain = [field for field in FIELDS if any(case.get(role + "_fields", {}).get(field, {}).get("confidence", 0) < .9
                                                   for role in ("si", "bl"))]
    case["uncertain_fields"] = uncertain
    if uncertain:
        case.update(status="NEEDS_REVIEW", review_reason="missing_value", internal_reason="LOW_EXTRACTION_CONFIDENCE")
        return
