...
def is_actionable_comparison_request(email):
    ...
    compare = bool(
        re.search(
            r"compare|verify|check\s+(?:the\s+)?si|si\s+and\s+(?:the\s+)?draft|confirm\s+(?:the\s+)?(?:si|documents?)",
            message,
        )
    )
...
def process_email(email, inbox, role_override=None, *, force_vision=False):
    ...
    if not paths:
        if not is_actionable_comparison_request(email):
            case.update(
                status="NOT_APPLICABLE", internal_reason="AWAITING_COMPARISON_DOCUMENTS"
            )
            return _submission_record(category), case
        case.update(
            status="NEEDS_REVIEW",
