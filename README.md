...
- **Category action pending:** An SI request, invoice query, general message, or spam classification is in one of its active category-specific workflow states.
...
Case Detail gives every category a state selector with realistic intermediate steps. Imported emails receive a conservative initial state from evidence in their subject and body: complete SI details can be ready to prepare, missing or pending information can be waiting, completed automated notices can be complete, and strong spam indicators can be confirmed spam. States that claim an operational action occurred, such as routed or response sent, require a reviewer decision. Reviewers can save the current state, create response drafts where appropriate, and move a task backward or forward as work changes. These reviewer decisions are stored separately and do not change the original competition classification or SI/BL verification output.

Search, category, workflow status, and date filters narrow the email table. With **All categories** selected, Workflow status stays compact with open category work, completed category work, no mismatch, mismatch, and human review. Selecting one category reveals only that category's detailed states. Empty states are omitted and every visible status includes its current email count, so a status choice corresponds to loaded emails. The date choices are **Today**, **Last 7 Days**, **Last 30 Days**, **All Time**, and **Custom range**. Records without meaningful timestamps are excluded instead of assigned invented dates. The seeded validation snapshot does not produce artificial Daily/Weekly/Monthly charts. Each compact table row shows the subject with its sender underneath, category, workflow status, attachment count, updated time, and the next action.
