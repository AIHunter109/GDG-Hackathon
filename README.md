...
The hosted browser edition also provides a batch uploader for mixed PDF/TXT files, a mixed folder, or separate SI and BL folders. It previews filename-based pairs before processing, keeps unclear or incomplete pairs out of verification, supports manual pairing, processes up to 10 pairs per run, and skips previously saved file signatures. Complete TXT documents are extracted locally; PDFs are sent to Gemini only after the reviewer starts processing.
