...
Search, category, workflow status, and date filters narrow the email table. The workflow-status choices change with the selected category: document comparison shows verification outcomes, while SI requests, invoice queries, general messages, and spam each show their own valid operational states. The date choices are **Today**, **Last 7 Days**, **Last 30 Days**, **All Time**, and **Custom range**. Records without meaningful timestamps are excluded instead of assigned invented dates. The seeded validation snapshot does not produce artificial Daily/Weekly/Monthly charts. Each compact table row shows the subject with its sender underneath, category, workflow status, attachment count, updated time, and the next action.
