...
Live site: [https://seal-509214.web.app](https://seal-509214.web.app). Sign in with the project owner Google account to load the 520 original bundle emails. The Dashboard leads with **Verify documents** and **Review exceptions**, followed by four clickable operational totals, concise validation results, a focused verification overview, review-reason filters, and the **Emails** table. The **Verify documents** workflow near the top supports one email, a mixed batch, or separate SI and BL folders; **New email history** remains separate. The source email's attachment count is shown as **Attachments**; the bundle has 394 emails with zero files, 2 with one file, and 124 with two files. Case Detail includes the seven SI/BL fields, mismatch source evidence, processing-method explanations, email and attachment text, reviewer actions, review completion undo, retry, and correction drafts. The 520 processed case records are in the private `bundleCases` Firestore collection. Reviewer edits and new email submissions are saved under `users/{uid}/cases`. Uploads recognized as documents from an original email are excluded from the dashboard. Open the original Case Detail and choose **Reanalyze with Gemini** to update that case instead of creating another entry.
...
The browser's final seven-field comparison is deterministic. Gemini helps extract selected PDF/TXT documents and explain review evidence. AI-derived values require review. The Python pipeline remains the competition scorer and supports TXT/XLSX/DOCX/PDF, uncertain intent, document-role and label fallback, and image-only PDF transcription with `GEMINI_API_KEY` configured. The hosted app accepts single or batch PDF/TXT uploads. Batch mode accepts mixed files, a mixed folder, or separate SI and BL folders; it pairs files by shared filename ID, previews ambiguous or missing pairs without processing them, reads complete TXT documents locally, and sends PDFs to Gemini only after the reviewer starts a run. Runs process 1–10 pairs at a time, can pause after the current pair, retry failures, and skip pairs already saved under the same file signature. Conflicting shipment identifiers go to human review. XLSX/DOCX batch files remain unsupported in the browser; the original bundle's XLSX/DOCX cases are displayed from private records processed by Python.
